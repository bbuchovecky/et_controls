...
def main(
    ...
    z_var = "ET"
    xb_var = "EVAPIDX"  # ET / P
    yb_var = "ARIDIDX"  # Rn / (Lv * P)

    if timeagg:
        stack_dims = ["lat", "lon", "year"]  # aggregate and compute equal-width bins over (lat, lon, year) dimensions
        time_tag = "agg"
    else:
        stack_dims = ["lat", "lon"]
        time_tag = "mean"
    ...
    fhist["FLDS_month_1"] = xclim.load_coupled_fhist_ppe("FLDS", "lnd", "month_1", "h0", keep_var_only=True)["FLDS"].sel(time=time_slice).reindex_like(grid, method="nearest", tolerance=1e-3)
    fhist["FIRE_month_1"] = xclim.load_coupled_fhist_ppe("FIRE", "lnd", "month_1", "h0", keep_var_only=True)["FIRE"].sel(time=time_slice).reindex_like(grid, method="nearest", tolerance=1e-3)
    fhist["FSDS_month_1"] = xclim.load_coupled_fhist_ppe("FSFS", "lnd", "month_1", "h0", keep_var_only=True)["FSDS"].sel(time=time_slice).reindex_like(grid, method="nearest", tolerance=1e-3)
    fhist["FSR_month_1"] = xclim.load_coupled_fhist_ppe("FSR", "lnd", "month_1", "h0", keep_var_only=True)["FSR"].sel(time=time_slice).reindex_like(grid, method="nearest", tolerance=1e-3)
    ...
    #   Rn = (LW_dn - LW_up) + (SW_dn - SW_up)
    #   Rn = (FLDS - FIRE) + (FSDS - FSR)
    fhist["RN_month_1"] = fhist["FLDS_month_1"] - fhist["FIRE_month_1"] + fhist["FSDS_month_1"] - fhist["FSR_month_1"]
    ...
    fhist["EVAPIDX_month_1"] = fhist["ET_month_1"] / fhist["PRECT_month_1"]
    fhist["EVAPIDX_month_1"].attrs["long_name"] = "evaporative index (ET / P)"
    fhist["EVAPIDX_month_1"].attrs["units"] = "1"

    # Monthly aridity index (Rn / Lv / P)
    fhist["ARIDIDX_month_1"] = fhist["RN_month_1"] / fhist["PRECT_month_1"]
    fhist["ARIDIDX_month_1"].attrs["long_name"] = "aridity index (Rn / Lv / P)"
    fhist["ARIDIDX_month_1"].attrs["units"] = "1"
    

    # Output directory
    case = f"TIME{time_tag}.{xb_var}xbin_{yb_var}ybin_d{z_var}mean"
    ...
    t0 = time.time()
    result = xclim.get_equalwidth_binned_mean(
        Z=z, xb=xb, yb=yb, xnb=nbin, ynb=nbin, stack_dims=stack_dims, agg_dims=["gridcell"])
    print(f"Equal-width binning: {time.time()-t0:.1f}s")    

    t0 = time.time()
    result.to_netcdf(rootdir / f"ebin{nbin}.{year_start}-{year_end}.{case}.{str_mem.zfill(3)}.nc")
    print(rootdir / f"ebin{nbin}.{year_start}-{year_end}.{case}.{str_mem.zfill(3)}.nc")
...
if __name__ == '__main__':
    ...
    parser.add_argument("--nbin", type=int, required=True, help="number of equal-width bins (x and y)")
    parser.add_argument("--timeagg", type=int, required=True, choices=[0, 1], help="1 for time aggregated equal-width bins, 0 for time mean")
