...
CATALOG_PATH = Path("/glade/campaign/univ/uwas0155/catalogs/cmip6_cesm_mon.csv")
REGRID_ROOT = Path("/glade/campaign/univ/uwas0155/cmip6/regridded")

VARIABLES = ["evspsbl", "tran", "lai", "pr", "rsds", "rlds"]
EXPERIMENT_ID = "historical"
SOURCE_IDS = ["CESM2", "CESM2-WACCM", "CESM2-FV2", "CESM2-WACCM-FV2"]
MEMBER_IDS = "top"
...
def main() -> None:
    ...
    for sid, vardict in data_dict.items():
        ...
        for var, da in vardict.items():
            ...
            # Get member ID
            mid_tag = ""
            if "member" in da_regridded.dims and len(da_regridded["member"]) == 1:
                if "member_id" in da_regridded.dims:
                    mid_tag = f"_{da_regridded['member_id'][0]}"
                else:
                    mid_tag = "_topmember"
            ...
            fname = f"{var}_{table_id}_{sid}_{EXPERIMENT_ID}{mid_tag}_gr1deg_{start_str}-{stop_str}.nc"
