...
computed per-member per-gridpoint.

Δ operator := difference between two consecutive years
(i.e., first-order finite difference of annual timeseries)

Predictand:
    ΔET   = annually averaged ET                     (EFLX_LH_TOT, W m-2)
Predictors:
...
if NORMALIZE_TARGET_BY_MEAN_TEMP:
    target = target / data_annual["TSA_month_1"].mean(dim='year')

n_coeffs = len(PREDICTORS) + 1

# Calculate first-order differences along year dimension
# dims: (lat, lon, year, member)
# shape: (n_lat, n_lon, n_year-1, n_member)
print("\nCalculating first-order difference...")
target_diff = target.diff(dim='year')
predictor_diff = {p: data_annual[p].diff(dim='year') for p in PREDICTORS}

# Retrieve coordinate metadata for output reconstruction
lat   = target_diff.lat
lon   = target_diff.lon
years = target_diff.year
members = target_diff.member
...
# target_diff shape after stack: (n_gridpoints, n_years, n_members)
print("Stacking spatial dimensions...")
target_stacked = (
    target_diff
    .stack(gridpoint=('lat', 'lon'))
    .transpose('gridpoint', 'year', 'member')
    .values  # (n_gridpoints, n_years, n_members)
)
...
predictor_stacked = np.stack(
    ...
    axis=-1,
)  # shape: (n_gridpoints, n_years, n_members, n_pred)

n_gridpoints = target_stacked.shape[0]

# Output array: (n_gridpoints, n_members, n_coeffs)
coeffs_out = np.full((n_gridpoints, n_members, n_coeffs), np.nan)
std_coeffs_out = np.full((n_gridpoints, n_members, n_coeffs), np.nan)

METRIC_NAMES = ["R2", "RMSE", "MAE"]
n_metrics = len(METRIC_NAMES)
metrics_out = np.full((n_gridpoints, n_members, n_metrics), np.nan)

print("Running per-gridpoint per-member OLS regression...")
for m_idx, m in enumerate(members.values):
    print(f"  Member {m}...")

    # y: (n_gridpoints, n_years)
    y = target_stacked[:, :, m_idx]

    # X_raw: (n_gridpoints, n_years, n_pred)
    X_raw = predictor_stacked[:, :, m_idx, :]

    for g in range(n_gridpoints):
        y_g = y[g]        # (n_years,)
        X_g = X_raw[g]    # (n_years, n_pred)

        # NaN mask: drop any year where target or any predictor is NaN
        ...
        std_coeffs_out[g, m_idx, :] = std_coeffs
        ...
        metrics_out[g, m_idx, :] = [r2, rmse, mae]

# Reshape output arrays back to (n_coeffs, n_members, n_lat, n_lon)
coeffs_out = coeffs_out.reshape(n_lat, n_lon, n_members, n_coeffs).transpose(3, 2, 0, 1)
std_coeffs_out = std_coeffs_out.reshape(n_lat, n_lon, n_members, n_coeffs).transpose(3, 2, 0, 1)
metrics_out = metrics_out.reshape(n_lat, n_lon, n_members, n_metrics).transpose(3, 2, 0, 1)
...
coeffs_da = xr.DataArray(
    ...
    attrs={
        ...
        "target":      "EFLX_LH_TOT (W m-2), first-order annual difference",
...
metrics_da = xr.DataArray(
    ...
    attrs={
        "description": "OLS goodness-of-fit metrics per gridpoint and member",
