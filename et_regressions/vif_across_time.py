...
computed per-member per-gridpoint across the PPE.
...
Inputs mirror multiple_regression_across_time.py exactly:
...
Output:
    vif_time_gridpoint_masked_1950-2014.nc
    Variables:
...
OUTNAME = "vif_time_gridpoint_masked_noFSDS_1950-2014.nc"
...
years = predictor_diff[PREDICTORS[0]].year
members = predictor_diff[PREDICTORS[0]].member
...
# Output arrays: (n_gridpoints, n_members, n_pred)
vif_out  = np.full((n_gridpoints, n_members, n_pred), np.nan)
r2aux_out = np.full((n_gridpoints, n_members, n_pred), np.nan)

print("Running per-gridpoint per-member auxiliary OLS regression for VIF...")
for m_idx, m in enumerate(members.values):
    print(f"  Member {m}...")

    for g in range(n_gridpoints):
         X_g = predictor_stacked[g, :, m_idx, :]  # (n_years, n_pred)
         ...
         for j in range(n_pred):
            ...
            X_aux = np.delete(X_g, j, axis=1)  # (n_years, n_pred-1)
            ...
            r2aux_out[g, m_idx, j] = r2_j
    
            # Clamp R^2 to [0, 1) to avoid division by zero or negative VIF
            r2_j_clamped = np.clip(r2_j, 0.0, 1.0 - 1e-10)
            vif_out[g, m_idx, j] = 1.0 / (1.0 - r2_j_clamped)

# Reshape to (n_pred, n_members, n_lat, n_lon)
vif_out   = vif_out.reshape(n_lat, n_lon, n_members, n_pred).transpose(3, 2, 0, 1)
r2aux_out = r2aux_out.reshape(n_lat, n_lon, n_members, n_pred).transpose(3, 2, 0, 1)

print("Building output Dataset...")
vif_da = xr.DataArray(
    data=vif_out,
    dims=["predictor", "member", "lat", "lon"],
    coords={
    ...
    attrs={
        ...
        "differencing":     "Predictors are first-order annual differences",
